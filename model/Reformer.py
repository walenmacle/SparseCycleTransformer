...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        """
        bucket_size and n_hashes are read from configs
        """
        super(Model, self).__init__()
        self.pred_len = configs.pred_len
        self.seq_len = configs.seq_len
        
        # Get Reformer-specific parameters with defaults
        bucket_size = getattr(configs, 'bucket_size', 4)
        n_hashes = getattr(configs, 'n_hashes', 4)
